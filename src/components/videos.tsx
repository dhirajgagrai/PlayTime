...
const Videos = (props: { pId:string, pd: PlaylistDetails, fd: number[] }): ReactElement => {
  ...
  const progress = totalDuration ? Math.floor(watchedDuration / totalDuration * 100) : 0

  const handleDelete = (duration: number, id: string, index: number) => {
  ...
  const handleWatched = (duration: number, id: string) => {
    if (watched.some((element) => element === id)) {
      setWatchedDuration(watchedDuration - duration)
    ...
    <div className="max-w-3xl mx-auto">
      <div className="flex justify-between mb-2">
        <div>
          ...
          <div className="text-gray-500">Total videos: {props.pd?.videos.length} {Boolean(unavailableVideoCount) && `(${unavailableVideoCount} unavailable)`}</div>
          ...
          videos.map((video, i) => (
            <div className="flex items-center justify-between" key={`video-div-${i}`}>
              ...
              <div className="flex basis-48 items-center gap-2">
                <Button className="text-gray-400 hover:text-gray-900" size="icon" variant="ghost" onClick={() => handleDelete(props.fd[i], video.id as string, i)}>
                  <TrashIcon className="h-4 w-4" />
                  <span className="sr-only">Delete</span>
                </Button>
                <Button className="text-gray-400 hover:text-gray-900" size="icon" variant="ghost" onClick={() => handleWatched(props.fd[i], video.id as string)}>
